...
export default function EditSalarySlip() {
        ...
        fetch(`/api/salary/${id}`)
            ...
            .then(fetchedData => {
                if (fetchedData.error) {
                    ...
                    const earnings = fetchedData.earnings || { basic: 0, hra: 0, conveyance: 0, special: 0, bonus: 0 };
                    const deductions = fetchedData.deductions || { epf: 0, pt: 0, advance: 0, leaveDeduction: 0, tds: 0 };
  ...
  // Auto-calculate Paid Days & Leave Deduction
  useEffect(() => {
      if (!loading) {
          ...
          // Recalculate Leave Deduction if basic is present
          const basic = data.earnings.basic || 0;
          if (basic > 0 && totalDays > 0) {
              const leaveDeductionAmount = Math.round((basic / totalDays) * leaves);
               // Update deduction only if it changed to avoid infinite loop if exact same
              if (data.deductions.leaveDeduction !== leaveDeductionAmount) {
                  setData(prev => ({
                      ...prev,
                      deductions: {
                          ...prev.deductions,
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row print:block">
        ...
        <div className="w-full md:w-[450px] bg-white border-r border-slate-200 flex-shrink-0 h-screen overflow-y-auto print:hidden shadow-xl z-10 block">
            <div className="p-4 border-b border-slate-100 sticky top-0 bg-white z-10 flex justify-between items-center">
                 ...
                 <h2 className="font-bold text-slate-800">Edit Payslip</h2>
            </div>
            ...
            <div className="p-6 space-y-8">
                ...
                <section className="space-y-4">
                    ...
                    <div className="space-y-3">
                        <div>
                            <label className="block text-xs font-medium text-slate-500 mb-1">Select Employee (to autofill)</label>
                            <select 
                            ...
                            >
                                <option value="">-- Change Employee --</option>
                                {employees.map(emp => (
